...
txt = os.path.join(path_txt,'ficha_tecnica.txt')
...
ficha={}
tipo=0
for l,linha in enumerate(txt):
    #agradecimentos
    if l in [2,]:
        ficha[linha]=[]
        tipo=l
        lista=ficha[txt[tipo]]
    elif l in [16,]:
        ficha[linha]={}
        lista=None
        tipo=l
        ...
        if tipo==2:
            ...
            if lista==None:
                if linha:
                    ficha[txt[tipo]][linha]=[]
                    lista=ficha[txt[tipo]][linha]
            else:
                if linha:
                    lista.append(linha)
                else:
...
for t,tipo in enumerate(ficha):
    tit=fmt_tit.copy()
    tit.append(tipo.upper()+'\n\n')
    fmt.append(tit)
    
    tipo=ficha[tipo]
    if not t:
        txt=fmt_txt.copy()
        for l,linha in enumerate(tipo):
            txt.append(linha+'\n')
        ...
        txt=fmt_txt.copy()
        for i,item in enumerate(tipo):
            print(item)
            txt.append(item+'\n')
            for l,linha in enumerate(tipo[item]):
                print(linha)
                txt.append(linha+'\n')
...
save()
translate(me,meio-ch)
textBox(fmt,(0,20*cm,cw,ch))
...
#                     pdf='pdf/grafica/0/obras/txt/%s-%s-%s.pdf' % (eixo,obra,lang)

#                     if grafica:
#                         if not os.path.isfile(pdf):
#                             pdf='pdf/grafica/0/obras/txt/%s-%s-%s.pdf' % (0,0,lang)
